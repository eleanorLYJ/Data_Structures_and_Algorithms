...
class hashStruct {
  ...
  private:
    vector<pair<int, int>>
        h; // note: class 裡的宣告時不可以有執行語句，so call constructer
    vector<int> vis;
    int mod = 2;
    int vec_size = 3;
    // static hash function
    int hash(int);
    // input: key ,output: the empty position, if (that output is -1) mean
    // caanot find!
    int linear_probing_to_find_empty_pos(int);
    // input: key, output: key position, if (that output is -1) mean caanot
    // find!
...
int hashStruct::hash(int key) { return key % mod; }
